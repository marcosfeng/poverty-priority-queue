...
Pre-existing literature suggests that relative material scarcity -- particularly wealth inequality -- is a significant contributing factor in dictating how people behave. We characterize irrational decision-making across impoverished groups by examining the effects of inefficiency in time allocation.
...
Our experimental design simulates the environment of an ice cream parlor using an interactive online game. The game was developed in PsychoPy version 2023.1.3 and hosted on the Pavlovia platform. Participants were ice cream parlor owners whose goal was to maximize profit by serving customers with varying needs.
...
2\. Importance (value): We equate importance with subjective value, represented by dollar signs on the image. A single dollar sign indicated low importance (yielding one point), while two dollar signs indicated high importance (yielding two points). The points were the participants' earnings, and each point was worth \$0.10. The total earnings, a proxy for the participant's performance, were displayed at the top of the screen.

3\. Urgency: We used clock icons on each image to convey the degree of urgency of a task. Two clock icons denoted a high-urgency order, while one clock icon represented a low-urgency order. High-urgency tasks lose value (dollar signs) if participants do not complete them in a given round (e.g., an urgent task worth 2 points would become an urgent task worth 1 point in the next round, and an urgent task worth 1 point would lose all of its value). If a high-urgency task lost all its value, it would disappear from the queue, and the participant was "punished" by having only one order in the queue for the following two rounds. Conversely, low-urgency tasks stayed in the queue for four rounds before being replaced with a new order, and there was no punishment if such a task disappeared.

In each round, participants saw the screen shown in @img-exp. They were instructed to select a task from the queue (left or right) and click on the matching ice cream icons in the center of the screen to complete it. If they make a mistake, they can use the trash can icon to discard the current order and start over. The game continued for a total of 40 orders. Participants were not restricted to a specific order of task completion; they could choose any task from the queue at any time. This open-choice design allowed us to observe and analyze the decision-making and prioritization strategies employed by the participants. Note that we incentivized players in the game by offering a prize of up to $\$4.00$ , contingent on the total number of points players accumulated throughout the game.

![Game interface for the game. Participants have a queue, with a task on each side, and a series of icons they must press in order to complete a task. The clocks and dollar signs on a task represent the urgency and importance of the task, respectively. The top of the screen displays a points counter and a rounds counter.](images/exp_example.png){#img-exp}
...
trade_off_accuracy <- demograph %>%
  dplyr::select("participant","S1701_C03_001E","S1903_C03_001E") %>%
  right_join(trade_off_accuracy,by = "participant") %>%
  ungroup() %>%
  mutate(poverty_zip = scale(as.numeric(S1701_C03_001E)),
         median_income_zip = scale(as.numeric(S1903_C03_001E)))
...
joint_pilot <- joint_pilot %>%
  mutate(chosen_i = ifelse(choice == "L", i1, i2),
         chosen_e = ifelse(choice == "L", e1, e2),
         chosen_u = ifelse(choice == "L", u1, u2),
         not_chosen_i = ifelse(choice == "L", i2, i1),
         not_chosen_e = ifelse(choice == "L", e2, e1),
         not_chosen_u = ifelse(choice == "L", u2, u1))
...
```{r}
#| label: pca-stai-urg-bias

ggplot(trade_off_accuracy, aes(x = stai, y = Urgency)) +
  labs(x="Anxiety", y="Urgent Choices") +
  geom_jitter(color = "blue", alpha = 0.4, width = 0.03, height = 0.01) +
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial"))

summary(pglm(formula = Urgency ~ (Dim.1 + Dim.2)*stai,
             data = trade_off_accuracy,
             effect = "twoways",
             index = c("participant", "round"),
             model = "random",
             family = binomial('logit'))) -> urg_bias_model
```
...
# RT histograms
### log(RT)
hist(as.numeric(trade_off_accuracy$RT), breaks = 100)
hist(log(as.numeric(trade_off_accuracy$RT)), breaks = 100)
ggplot(trade_off_accuracy, aes(x = as.numeric(income), y = log(as.numeric(RT)))) +
  labs(x="Income", y="Log of RT in Trade-offs") +
  geom_jitter(color = "blue", alpha = 0.4, width = 0.03, height = 0.01) +
  geom_smooth(method = "glm", 
              method.args = list(family = "gaussian"))

ggplot(trade_off_accuracy, aes(x = stai, y = log(as.numeric(RT)))) +
  labs(x="Anxiety", y="Log of RT in Trade-offs") +
  geom_jitter(color = "blue", alpha = 0.4, width = 0.03, height = 0.01) +
  geom_smooth(method = "glm", 
              method.args = list(family = "gaussian"))

summary(glm(formula = as.numeric(RT) ~ Dim.1*stai*Dim.2, data = trade_off_accuracy, family = "gaussian"))
```
...
```{r}
# Histogram of effort cost
hist(costs$cL, breaks=16)
hist(costs$cH, breaks=12)
   #Plot
...
@accuracy-histogram highlights how participants typically make optimal decisions (when given 'challenging' tasks) with a probability in between 0.5 and 0.6. This is generally what we expect to observe.
...
```{r}
#| label: aggregate-accuracy-results
#| fig-cap: "Aggregate Accuracy by Income and Anxiety"
#| fig-subcap:
#|   - "Prolific, Income"
#|   - "Prolific, Anxiety"
#| layout-ncol: 2
...
#| fig-cap: "Trial Accuracy by Income and Anxiety (logistic function fit)"
#| fig-subcap:
#|   - "Accuracy in trade-off decisions as a function of self-reported income"
#|   - "Accuracy in trade-off decisions as a function of self-reported anxiety, by the STAI questionnaire"
#| layout-ncol: 2
#| column: page-right

# plot with Income
ggplot(trade_off_accuracy, aes(x = as.numeric(income), y = as.numeric(Optimal))) +
...
# Now with STAI
ggplot(trade_off_accuracy, aes(x = stai, y = as.numeric(Optimal))) +
  labs(x="Anxiety", y="Accuracy") +
  geom_jitter(color = "blue", alpha = 0.4, width = 0.03, height = 0.01) +
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial")) 
```
...
### ¶Redesign Simulations and Comparisons
...
[![Source: https://todoist.com/productivity-methods/eisenhower-matrix](images/eisenhower-matrix.png){width="5.36in"}](https://todoist.com/productivity-methods/eisenhower-matrix)
...
```{r}
#| label: glm-accuracy
summary(pglm(formula = Optimal ~ as.numeric(income)*stai,
             data = trade_off_accuracy,
             effect = "twoways",
             index = c("participant", "round"),
             model = "random",
...
summary(pglm(formula = Optimal ~ (Dim.1 + Dim.2)*stai,
             data = trade_off_accuracy,
             effect = "twoways",
             index = c("participant", "round"),
             model = "random",
             family = binomial('logit')))
### With demographics
summary(pglm(formula = Optimal ~ age + as.numeric(education) + as.numeric(income),
             data = trade_off_accuracy,
             effect = "twoways",
             index = c("participant", "round"),
             model = "random",
...
summary(pglm(formula = Optimal ~ age + as.numeric(education) + poverty_zip,
             data = trade_off_accuracy,
             effect = "twoways",
             index = c("participant", "round"),
             model = "random",
             family = binomial('logit')))
summary(pglm(formula = Optimal ~ age + as.numeric(education) + median_income_zip,
             data = trade_off_accuracy,
             effect = "twoways",
             index = c("participant", "round"),
             model = "random",
...
## ¶Urgency Bias and Deprivation Measures

```{r}
#| label: img-urg-pca

#####  trade-off decisions correlated with PCA deprivation
ggplot(trade_off_accuracy, aes(x = Dim.1, y = Urgency)) +
  labs(x = "Dimension 1", y="Urgent Choices") +
  geom_jitter(color = "blue", alpha = 0.4, width = 0.03, height = 0.01) +
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial"))

#####  trade-off decisions correlated with PCA deprivation
ggplot(trade_off_accuracy, aes(x = Dim.2, y = Urgency)) +
  labs(x="Dimension 2", y="Urgent Choices") +
  geom_jitter(color = "blue", alpha = 0.4, width = 0.03, height = 0.01) +
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial"))


## Urgency ~ anxiety#income + round
### With psychological mechanisms
summary(lm(Urgency ~ income*stai, data = trade_off_accuracy))
summary(glm(as.numeric(Urgency) ~ income*stai, data = trade_off_accuracy, family = "binomial"))
### With demographics
summary(lm(Urgency ~ age + education + income, data = trade_off_accuracy))

# Zip codes
#plot with Income
ggplot(trade_off_accuracy, aes(x = S1701_C03_001E, y = as.numeric(Urgency))) +
  labs(x="ZIP code deprivation score", y="Urgency") +
  geom_jitter(color = "blue", alpha = 0.4, width = 0.03, height = 0.01) +
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial")) 
ggplot(trade_off_accuracy, aes(x = S1903_C03_001E, y = as.numeric(Urgency))) +
  labs(x="ZIP code median income", y="Urgency") +
  geom_jitter(color = "blue", alpha = 0.4, width = 0.03, height = 0.01) +
  geom_smooth(method = "glm", 
              method.args = list(family = "binomial"))
